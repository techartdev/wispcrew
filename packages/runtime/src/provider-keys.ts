 ...
 *
 * WispCrew lets each agent choose its own provider and model, which only
 ...
 *
 * Keys therefore live under `WISPCREW_KEY_<preset>` in the encrypted store,
...
export const LEGACY_KEY = 'WISPCREW_API_KEY';
...
export function providerSecretKey(presetId: string): string {
  return `WISPCREW_KEY_${presetId.toUpperCase().replace(/[^A-Z0-9]+/g, '_')}`;
}
 ...
 *
 * Runs once at startup. The key is copied to `WISPCREW_KEY_<preset>` and the
...
export function retireLegacyKeyIfRedundant(userDataDir: string): void {
  ...
  const hasOwn = Object.keys(secrets).some(
    (name) => name.startsWith('WISPCREW_KEY_') && secrets[name],
  );
  if (!hasOwn) return;
